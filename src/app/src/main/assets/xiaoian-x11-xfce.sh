...
# :0 ...". The two older patterns are upstream Termux:X11's process names and
# never match here; without the first one the X server outlived every stop,
# since closing its window no longer kills it, and the next start found :0
# taken ("server already running"). No spaces: the list is word-split.
DE_APP_PACKAGE="$APP_PACKAGE"
DE_SESSION_PROC="xfce4-session"
DE_HOST_NAMES="pulseaudio"
DE_HOST_PATTERNS='^app_process.*com\.termux\.x11\.CmdEntryPoint ^termux-x11.com\.termux\.x11 com\.termux\.x11\.Loader'
...
do_start() {
    ...
    if [ -n "$(chroot_pids_owned_by app)" ] && [ -z "$(chroot_pids_owned_by foreign)" ]; then
        ...
        # Only the terminals are spared: a previous session's X server would
        # otherwise keep :0 and fail this start.
        stop_host_procs TERM
        sleep 1
        stop_host_procs KILL
    else
...
do_relaunch() {
    ...
    if [ "$(saved_mode)" = "extend" ]; then
        ...
        echo "[*] Launching Termux:X11 on the built-in display..."
        /system/bin/am start -n "$APP_PACKAGE"/com.termux.x11.MainActivity --display 0
    fi
}
