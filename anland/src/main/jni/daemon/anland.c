...
#include "../anland_core/common/protocol.h"
#include "../anland_core/common/socket_utils.h"
...
static const char *default_socket_path(void)
{
    ...
    if (!tmpdir || tmpdir[0] == '\0')
        tmpdir = "/data/data/com.xiaoian.app/files/tmp";

    if (snprintf(path, sizeof(path), "%s/anland.sock", tmpdir) >=
        (int)sizeof(path)) {
